-- TrailBlaze CRM — Migration 004 (FIXED)
-- Run the cleanup query FIRST if you got errors on a previous attempt:
--   DROP TABLE IF EXISTS sequence_send_log CASCADE;
...
-- ============================================
-- 6. SNIPPETS (org_id NULLABLE for system defaults)
-- ============================================
...
CREATE POLICY snippets_org ON snippets FOR ALL USING (org_id IS NULL OR org_id = auth_user_org_id());
...
-- ============================================
-- SEED: System default snippets (no org_id)
-- ============================================
INSERT INTO snippets (org_id, title, shortcut, content, category, channel, is_shared) VALUES
(NULL, 'Quick check-in', '/checkin', 'Hi {first_name}, hope you''re doing well! Just wanted to check in and see how things are going at {company_name}. Anything I can help with?', 'follow_up', 'whatsapp', TRUE),
(NULL, 'QBR agenda', '/qbr', E'Hi {first_name},\n\nLooking forward to our quarterly review. Here''s what I''d like to cover:\n\n1. Key wins and milestones from last quarter\n2. Current challenges or blockers\n3. Your priorities for next quarter\n4. How we can better support {company_name}\n5. Contract and renewal discussion\n\nPlease feel free to add anything. See you on [date].\n\nBest,\n{account_manager_name}', 'meeting', 'email', TRUE),
(NULL, 'Thank you after meeting', '/thanks', E'Hi {first_name}, thank you for taking the time to meet today. Here''s a quick summary of what we discussed and the next steps we agreed on:\n\n[Summary here]\n\nI''ll follow up on these by [date]. Let me know if I missed anything.\n\nBest,\n{account_manager_name}', 'follow_up', 'email', TRUE),
(NULL, 'Renewal intro', '/renew', 'Hi {first_name}, your contract with us is coming up for renewal in [X] days. I wanted to get ahead of it and schedule a quick chat to review how things have been going and discuss next steps. Would any time next week work? {meeting_link}', 'renewal', 'email', TRUE),
(NULL, 'Gentle nudge', '/nudge', 'Hey {first_name}, just a gentle follow-up on my last message. I know things get busy — whenever you have a moment, I''d love to connect. No rush at all.', 'follow_up', 'whatsapp', TRUE);
